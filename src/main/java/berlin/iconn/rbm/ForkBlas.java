...
public class ForkBlas {
    private final static ForkJoinPool pool = ForkJoinPool.commonPool();

    private ForkBlas() {

    }

    public static void pmmuli(FloatMatrix self, FloatMatrix other, FloatMatrix result) {
        pool.invoke(new MulitplyPartly(self, other, result, 0, self.getRows(), 0, other.getColumns()));
    }

    private static final class MulitplyPartly extends RecursiveAction {
